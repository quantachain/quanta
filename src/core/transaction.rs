...
mod tests {
    ...
    fn test_transaction_creation() {
        let tx = Transaction::new(
            ...
            100_000_000, // 100 QUA in microunits
            1234567890,
        );
        assert_eq!(tx.amount, 100_000_000);
        assert_eq!(tx.fee, 1000); // 0.001 QUA
    ...
    fn test_account_state() {
        ...
        utxo_set.add_utxo(&tx, 0, 100);
        assert_eq!(utxo_set.get_balance("address1"), 50_000_000);
        
        // Spend 30 QUA - remaining 20 QUA stays in account
        assert!(utxo_set.spend_utxos("address1", 30_000_000));
        assert_eq!(utxo_set.get_balance("address1"), 20_000_000); // Proper change!
        
        // Verify nonce incremented
        assert_eq!(utxo_set.get_nonce("address1"), 1);
    }
}
