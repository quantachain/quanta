...
mod tests {
    ...
    fn test_passphrase_changes_derivation() {
        let mnemonic = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon art";
        let wallet_no_pass = HDWallet::from_mnemonic(mnemonic.to_string(), "");
        let wallet_with_pass = HDWallet::from_mnemonic(mnemonic.to_string(), "secret123");
        
        // Different passphrase = different wallet
        assert_ne!(wallet_no_pass.seed, wallet_with_pass.seed);
        assert_ne!(wallet_no_pass.master_key, wallet_with_pass.master_key);
    }
    
    #[test]
    fn test_encrypt_decrypt_roundtrip() {
        let mut wallet = HDWallet::new();
        wallet.generate_account(Some("Test".to_string()));
        
        let password = "super_secret_password";
        let encrypted = wallet.export_encrypted(password).unwrap();
        let decrypted = HDWallet::import_encrypted(&encrypted, password).unwrap();
        
        assert_eq!(wallet.mnemonic, decrypted.mnemonic);
        assert_eq!(wallet.accounts.len(), decrypted.accounts.len());
        assert_eq!(wallet.accounts[0].address, decrypted.accounts[0].address);
    }
    
    #[test]
    fn test_wrong_password_fails() {
        let wallet = HDWallet::new();
        let encrypted = wallet.export_encrypted("correct").unwrap();
        let result = HDWallet::import_encrypted(&encrypted, "wrong");
