...
impl Peer {
    ...
    pub async fn send_raw_data(&self, data: Vec<u8>) -> Result<(), String> {
        ...
        // If a timeout occurs, we must set last_seen = 0 BEFORE releasing the lock.
        // If we drop the lock before setting last_seen = 0, another thread can grab the lock
        // and write to the corrupted stream before we mark it dead.
        ...
        // 10-second timeout
        match tokio::time::timeout(std::time::Duration::from_secs(10), write_future).await {
            Ok(Ok(_)) => Ok(()),
            Ok(Err(e)) => {
                let mut info = self.info.write().await;
                info.last_seen = 0;
                Err(format!("Failed to write: {}", e))
            }
            Err(_) => {
                let mut info = self.info.write().await;
                info.last_seen = 0;
