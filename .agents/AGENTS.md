...
3. **Version Bumps**: Bump the protocol versions and network magics in necessary files (e.g., `src/network/protocol.rs`) when breaking network compatibility. You MUST explicitly bump the version in `Cargo.toml` and `Dockerfile`, and you MUST run `cargo check` before you commit, push, and tag.
