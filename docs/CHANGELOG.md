...
- **AddrMan: operators stuck at 1 peer — root cause (critical)**: All operators reported connecting to only 1 peer (the bootstrap node) despite 20+ validators being online. The root cause was the AddrMan `verified` flag introduced in v3.1.0: a peer could only be promoted to `verified=true` after a successful **outbound** connection. The bootstrap node (`node1`) only has **inbound** connections from validators — it never dials out to them — so every validator's entry in node1's discovery table stayed `verified=false` forever. When operators sent a `GetAddr` request to node1, it returned an empty list (only verified peers are gossiped). Result: zero peer discovery, everyone stayed at 1 peer. **Fix**: inbound peers that pass TLS + Falcon-512 handshake and self-report a `listen_addr` (not ephemeral source port) are now marked `verified=true` immediately. This allows the relay to gossip them to the network so operators discover each other directly.
- **"Backup state behind unit collection state" crash loop (root cause)**: AlephBFT would crash with `ERROR Backup state behind unit collection state. Next round inferred from: collection: N, backup: 0` whenever the auto-wipe fired on an oversized backup file. The wipe deleted the file, but the session was restarted with the **same `session_id`**. AlephBFT then read an empty backup (round=0) while its in-memory collection was already at round N from the previous run, causing an immediate fatal error that killed the consensus service. This looped indefinitely: wipe → crash → 30s sleep → restart → wipe → crash. **Fix**: after any backup wipe, `session_id` is incremented by +1 so AlephBFT always enters a completely fresh session with no in-memory state to reconcile.
- **Watchdog → stuck session restart loop (high CPU)**: The WATCHDOG that killed stuck sessions (no block in 600s) did not wipe the oversized backup before terminating. On restart, the same session would reload the large backup, immediately re-enter the stuck state, and the WATCHDOG would fire again — keeping CPU pinned at ~100%. **Fix**: the WATCHDOG now wipes the backup file (if > 5 MB) before sending the terminator signal. The session restart loop then detects the missing file, bumps `session_id`, and starts a brand-new AlephBFT session from round 0.
- **Docker CPU cap reduced 3.0 → 2.0**: The previous 3.0 core limit allowed the AlephBFT spin loop to consume 118% CPU (all 3 cores) and trigger Docker OOM restarts. Reduced to 2.0 cores so the node stays within limits without getting restarted by Docker, while still providing enough headroom for normal block production.
- **`quanta.toml` mounted into container**: `docker-compose.yml` now mounts `./quanta.toml` into the container at `/home/quanta/quanta.toml` so operators can adjust `bootstrap_nodes`, `dns_seeds`, and other settings without rebuilding or re-pulling the image.
...
- **Sender simplification**: The snapshot sender now simply serves its on-disk checkpoint state without any root cross-checking, since the forward-verification on the receiver side provides full security.
- **Protocol Bump (v43)**: Network isolated from nodes running v42.
